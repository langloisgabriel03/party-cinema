 ...
 */
export default function MagnetLink({ title, year, compact = false }) {
  ...
  return (
    <a
      ...
      className={`absolute bottom-1 left-1 flex ${compact ? 'size-7 text-sm sm:size-9 sm:text-base' : 'size-9 text-base'} items-center justify-center rounded-full bg-black/60 backdrop-blur-sm hover:bg-black/80`}
    >
      🧲
    </a>
  )
